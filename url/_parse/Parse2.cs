...
namespace nilnul.web.url
{
	...
	/// 
	/// </summary>
	public class _ParseX2
	{

		static public Url2 Parse(string s) {
			...
			if (authEnd<0)
			{
				return  Url2.CreateFroSchemeAuthority(
					scheme
					,
					_url.Authority1.Parse(toBeParsed)
				);
			}
			
			var authorityTxt = toBeParsed.Substring(0, authEnd);

			var authority = _url.Authority1.Parse( authorityTxt);
			...
			//{
				...
				var division = _url.division._ParseX.Parse(toBeParsed);
			//}
